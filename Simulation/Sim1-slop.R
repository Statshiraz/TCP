...
chpoinchek3<-function(PData,chp){
  k<-length(chp)
  if(k==0) {
    qq2=err2=qq1=pp1=hh=vv=err=NULL
  }
  if(k==1) {
    hh<-c(chp,0)
    err<-rep(0,0)
    ...
    if(dif<(max(err)/5)){
      vv=NULL
      pp1<-2
      qq2=qq1<-which(hh[pp1]!=0)
      err1=err2<-err[pp1]
    }
    if(dif>=(max(err)/5)){
      vv=NULL
      pp1<-which.min(err)
      qq2=qq1<-hh[which(hh[pp1]!=0)]
      err1=err2<-err[pp1]
    }
  }
  if(k>=2) {
    ...
    qq1<-hh[pp1,]
    err1<-err[pp1]
    dd=which((err1-min(err1))<5)
    if(length(dd)==1) {
      dd1<-dd
      qq2=qq1[dd1,][qq1[dd1,]!=0]
      err2=err1[dd1]
    }
    if(length(dd)>=2){ 
      dd1<-which(apply((qq1[dd,]!=0),1,sum)==min(apply((qq1[dd,]!=0),1,sum)))
      qq2<-round(qq1[dd[dd1],][qq1[dd[dd1],]!=0])
      err2<-err1[dd[dd1]]
    }
  }
  list(qq2,err2,qq1,err1)
}
