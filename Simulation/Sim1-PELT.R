...
CL2<-function(PData,chp){
  n=length(PData)
  chp=c(0,chp,n)
  ybar=rep(0,0)
  for(i in 1:(length(chp)-1)){
    ybar[i]=abs(mean(PData[(chp[i]+1):chp[(i+1)]]))
  }
  if(length(ybar)==1) out=1
  if(length(ybar)==2) out=abs(ybar[1]-ybar[2])
  if(length(ybar)>2){
    yl=rep(0,0)
    yl[1]=abs(ybar[1]-ybar[2])
    for(i in 2:(length(ybar)-1)) yl[i]=abs(ybar[i]-ybar[(i+1)])
    out=sum(yl)
  }
  out
}
...
PELT<-function(PData,CF){
  ...
  chp<-(L[[n+1]][-1]-1)
  chp=chp[which(chp>3&chp<(n-3))]
  err<-sqrt(sum((trend(PData,chp)[[2]]-PData)^2))
  list(chp,err)
}
...
yhatf<-function(Data,s){ 
  hh=chpdata=chpdatanew=chpdatanew1=NULL
  P<-ncol(Data)
  ...
  if(P!=1){
    ...
    if(rp[[1]][2,4]<=0.05){
      chpdata<-PELT(PData,CL)[[1]]
      yhat<-trendest(Data,chpdata)
    }
...
for (m in 1:M){
  for(j in 1:R){
    ...
    err1[j]<-sqrt(sum((y1-yhat1[[j]][[1]][[3]])^2))
    err2[j]<-sqrt(sum((y2-yhat2[[j]][[1]][[3]])^2))  
    err3[j]<-sqrt(sum((y1-yhat3[[j]][[1]][[3]])^2))
    err4[j]<-sqrt(sum((y2-yhat4[[j]][[1]][[3]])^2))
    err5[j]<-sqrt(sum((y1-yhat5[[j]][[1]][[3]])^2))
    err6[j]<-sqrt(sum((y2-yhat6[[j]][[1]][[3]])^2))
    err7[j]<-sqrt(sum((y1-yhat7[[j]][[1]][[3]])^2))
    err8[j]<-sqrt(sum((y2-yhat8[[j]][[1]][[3]])^2))
